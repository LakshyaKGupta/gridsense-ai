...
/* Floating icon items scattered across the full hero */
const FLOATS = [
  { icon: '⚡', label: 'Load Spike', val: '+96%', color: '#FF4757', x: '8%',  y: '18%', delay: 0.8 },
  { icon: '🔋', label: 'Charging Active', val: '247 EVs', color: '#00E5A0', x: '82%', y: '14%', delay: 1.0 },
  { icon: '📊', label: 'Demand Forecast', val: '24h ahead', color: '#00C8FF', x: '88%', y: '52%', delay: 1.3 },
  { icon: '🔮', label: 'AI Confidence', val: '92%', color: '#A78BFA', x: '6%',  y: '68%', delay: 1.1 },
  { icon: '🏙', label: 'Zones Monitored', val: '198', color: '#F59E0B', x: '78%', y: '78%', delay: 1.5 },
  { icon: '📉', label: 'Peak Reduced', val: '−23 kW', color: '#00E5A0', x: '14%', y: '42%', delay: 1.6 },
]
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const onEnd = () => {
      // Hold on last frame 5s → fade → restart
      setTimeout(() => {
        setOpacity(0)
        setTimeout(() => {
  ...
  return (
    <section id="hero" style={{
      ...
      {/* Layered gradient mask — darkens edges, kills VEO logo bottom-right */}
      <div style={{
        position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none',
        background: `
          linear-gradient(to top,    rgba(8,12,18,1) 0%,  rgba(8,12,18,0.7) 14%, transparent 38%),
          linear-gradient(to bottom, rgba(8,12,18,0.7) 0%, transparent 20%),
          linear-gradient(to right,  rgba(8,12,18,0.55) 0%, transparent 28%),
          linear-gradient(to left,   rgba(8,12,18,0.45) 0%, transparent 22%)
        `,
      }} />
      ...
      {/* Floating metric icons — distributed all over hero */}
      {FLOATS.map((f, i) => (
        <motion.div
          key={i}
          initial={{ opacity: 0, scale: 0.7, y: 12 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ delay: f.delay, duration: 0.6, ease: [0.16,1,0.3,1] }}
          style={{
      ...
      >
        ...
        <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', letterSpacing: '1.5px' }}>Scroll</span>
      </motion.div>
    </section>
  )
}
