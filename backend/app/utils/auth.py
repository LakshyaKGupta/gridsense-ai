...
async def get_current_session(
    ...
    # Accept any demo token — these are local base64 JWTs generated by session.ts fallback
    if credentials.credentials.startswith("demo-") and credentials.credentials.endswith("-token"):
        # Decode the local base64 JWT to get the role
        try:
            import base64, json as _json
            parts = credentials.credentials.split(".")
            if len(parts) >= 2:
                padded = parts[1] + "=" * (4 - len(parts[1]) % 4)
                claims = _json.loads(base64.b64decode(padded).decode())
                return {
                    "sub": claims.get("sub", "demo@gridsense.ai"),
                    "role": claims.get("role", "operator"),
                    "email": claims.get("sub", "demo@gridsense.ai"),
                    "uid": claims.get("uid", "demo"),
                    "is_demo": True,
                }
        except Exception:
            pass
        # Fallback if decode fails: infer role from token name
